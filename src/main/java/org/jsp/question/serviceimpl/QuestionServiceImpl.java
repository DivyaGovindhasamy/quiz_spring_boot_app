...
import org.jsp.question.dto.QuizTake;
...
//		structure.setBody(question);
		return ResponseEntity.status(HttpStatus.OK).body(ResponseStructure.builder()
				...
				.message(" Questions saved  Successfully").body(question).build());
	...
	public ResponseEntity<?> submitQuiz(List<QuizResponse> quizResponses) {
		...
		if(q.isEmpty())
			throw InvalidQuestionIdException.builder().message("In valid Question Id Unable To calculate Result").build();
